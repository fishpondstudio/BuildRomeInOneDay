...
glob("./assets/*.ttf", (err, files) => {
   ...
   const preferredNames = files.map((file) => {
      ...
      generateBMFont(preferredPath, { filename: preferredName, smartSize: true }, (err, textures, font) => {
         ...
         textures.forEach((t) => writeFileSync(`./src/fonts/${path.basename(t.filename)}.png`, t.texture));
         writeFileSync(`./src/fonts/${path.basename(font.filename)}`, font.data);
      });
      return preferredName;
   });
   const scriptContent = ["// This file is generated by running `npm run font`. Do not modify directly"];
   preferredNames.forEach((f) => {
      scriptContent.push(`import ${f} from "../../fonts/${f}.fnt?url";`);
   });
   ...
   scriptContent.push("};");
   scriptContent.push(`export const fontBundle = { ${preferredNames.join(",")} };`);
